...
type SFUCapableAgent = VoiceAgentBase & {
  fetch(request: Request): Response | Promise<Response>;
  onRequest(request: Request): Response | Promise<Response>;
};
type SFUVoiceAgentLike = Constructor<SFUCapableAgent & VoiceAgentMixinMembers>;

interface SFUVoiceAgentMixinMembers extends VoiceAgentMixinMembers {
  getSFUConfig(): SFUConfig;
}

type SFUVoiceAgentMixinReturn<TBase extends Constructor<SFUCapableAgent>> =
  TBase &
    // oxlint-disable-next-line @typescript-eslint/no-explicit-any
    (new (...args: any[]) => SFUVoiceAgentMixinMembers);

export function withSFUVoiceTransport<TBase extends SFUVoiceAgentLike>(
  Base: TBase,
  ...
  class SFUVoiceAgentMixin extends Base {
    ...
    #getSFUTransport(): SFUVoiceTransport {
      // Agent.ctx is protected but present on every compatible Agent instance.
      const agent = this as unknown as { ctx: DurableObjectState };
      const storage = agent.ctx.storage;
      this.#transport ??= new SFUVoiceTransport({
        config: this.getSFUConfig(),
        routePrefix: normalizedPrefix,
        inputSampleRate,
        loadState: async () =>
          (await storage.get<SFUVoiceState>(SFU_STATE_KEY)) ?? null,
        saveState: async (state) => {
          if (state) {
            await storage.put(SFU_STATE_KEY, state);
          } else {
            await storage.delete(SFU_STATE_KEY);
          }
        }
      });
...
export function withSFUVoice<TBase extends Constructor<SFUCapableAgent>>(
  Base: TBase,
