...
describe("TwilioAdapter.handleRequest", () => {
  ...
  it.each([
    ...
    async (sampleRate, byteLength, outputLength) => {
      ...
      expect(
        harness.serverSocket.jsonSent.find(
          (message) => message.event === "media"
        )
      ).toMatchObject({ event: "media", streamSid: "stream-1" });
    }
  );
  ...
  it("forwards declared mulaw/8 kHz bytes unchanged", async () => {
    ...
    await harness.agentSocket.emit("message", { data: mulaw.buffer });
